    ...
    private suspend fun shareKey(session: MXOutboundSessionInfo,
        ...
        devicesByUsers
                ...
                .map { devices ->
                    ...
                    cryptoCoroutineScope.async { shareUserDevicesKey(session, devices.groupBy { it.userId }) }
                }.let { jobs ->
        ...
        devicesByUser.forEach { entry ->
            ...
            entry.value.forEach { deviceInfo ->
                ...
                if (olmSession?.sessionId == null) {
                    ...
                    contentMap.setObject(userId, deviceId, messageEncrypter.encryptMessage(payload, listOf(deviceInfo)))
                    haveTargets = true
                }
            }
        }
